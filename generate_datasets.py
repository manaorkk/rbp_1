import sys
import os
sys.path.append('/home/bert-rbp/motif')
...
def createkmers(args):
    ...
    #     lines = f.readlines()
    lines = os.listdir(args.path_to_benchmark)
    filename = 'EMPTY'
    i = 0
    while i < len(lines):
        line = lines[i]
        ...
        if match:
            filename = re.sub('[.]', '_', match.group())
        else:
            print("MatchError: the file name is not matched")
            i+=1
            continue
        # print(filename)
        value = 0
        ...
        if os.path.exists(filepath):
            with open(filepath, mode='a') as f:
                rawfile = os.path.join(args.path_to_benchmark,line)
                with open(rawfile, mode='r') as newf:
                    sublines=newf.readlines()
                j=0
                while j<len(sublines):
                    if not sublines[j][0] == '>':
                        f.write(seq2kmer(re.sub('U', 'T', sublines[j][:-1]), args.kmer) + '\t' +str(value) + '\n')
                    j+=1
...
def preprocess(args):
    ...
    for rbp in dirlist:
        ...
        #cut the dataset in which the number is over max number
        ...
        #the evaluation set from training set
        ...
        #the evaluation set and testing set
